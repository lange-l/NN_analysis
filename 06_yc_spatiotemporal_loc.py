...
--> localized vs nonlocalized (overall) NO CLUSTERS FOUND
--> undetected, localized vs nonlocalized (overall) NO CLUSTERS FOUND
...
# list of conditions names/descriptions:
cond_names = ['loc/anat', 'noloc/anat', 'loc/undet/anat', 'noloc/undet/anat',
              'loc/cross', 'noloc/cross', 'loc/undet/cross', 'noloc/undet/cross',
              'loc/overall', 'noloc/overall', 'loc/undet/overall', 'noloc/undet/overall']

# enter indices of conditions to be compared:
cond1 = 10
cond2 = 11
...
sensors = ['AF7', 'AF3', 'AFz', 'F1', 'F5', 'FT7', 'FC3', 'C1', 'C5', 'TP7', 'CP3', 'P1', 'P5', 'PO7', 'PO3', 'POz',
           ...
           'Fp1', 'Fz', 'F3', 'F7', 'FT9', 'FC5', 'FC1', 'C3', 'T7', 'CP5', 'CP1', 'Pz', 'P3', 'P7', 'O1', 'Oz', 'O2',
           'P4', 'P8', 'CP6', 'CP2', 'Cz', 'C4', 'T8', 'FT10', 'FC6', 'FC2', 'F4', 'F8', 'Fp2']
...
#  'F7', 'FT9', 'FC5', 'FC1', 'C3', 'T7', 'CP5', 'CP1', 'Pz', 'P3', 'P7', 'O1', 'Oz', 'O2', 'P4', 'P8',
#  'CP6', 'CP2', 'Cz', 'C4', 'T8', 'FT10', 'FC6', 'FC2', 'F4', 'F8', 'Fp2']

# save plot as
filename = 'undet_loc-noloc_overall_parietal'
...
# empty lists, one per condition
matrices_for_matrix = [[], [], [], [], [], [], [], [], [], [], [], []] # 2D arrays
evokeds_for_GrAv = [[], [], [], [], [], [], [], [], [], [], [], []] # 3D matrices

# list with conditions to be compared:
conds_to_plot = [cond_names[cond1], cond_names[cond2]]
...
for subj in subj_names:
    ...
    epochs.drop_channels(['VEOG', 'ECG'])
    epochs = epochs.pick(sensors)

    # Select epochs:
    # anatomical
    epo_loc_anat = epochs[(epochs.metadata.loca == 'hit') & (epochs.metadata.block_type == 1)]
    epo_noloc_anat = epochs[(epochs.metadata.loca == 'miss') & (epochs.metadata.block_type == 1)]
    epo_loc_undet_anat = epochs[((epochs.metadata.det == 'missF1') | (epochs.metadata.det == 'missF2')) &
                                (epochs.metadata.loca == 'hit') & (epochs.metadata.block_type == 1)]
    epo_noloc_undet_anat = epochs[((epochs.metadata.det == 'missF1') | (epochs.metadata.det == 'missF2')) &
                                  (epochs.metadata.loca == 'miss') & (epochs.metadata.block_type == 1)]
    # crossed
    epo_loc_cross = epochs[(epochs.metadata.loca == 'hit') & (epochs.metadata.block_type == 2)]
    epo_noloc_cross = epochs[(epochs.metadata.loca == 'miss') & (epochs.metadata.block_type == 2)]
    epo_loc_undet_cross = epochs[((epochs.metadata.det == 'missF1') | (epochs.metadata.det == 'missF2')) &
                                 (epochs.metadata.loca == 'hit') & (epochs.metadata.block_type == 2)]
    epo_noloc_undet_cross = epochs[((epochs.metadata.det == 'missF1') | (epochs.metadata.det == 'missF2')) &
                                   (epochs.metadata.loca == 'miss') & (epochs.metadata.block_type == 2)]
    #overall
    epo_loc_overall = epochs[(epochs.metadata.loca == 'hit')]
    epo_noloc_overall = epochs[(epochs.metadata.loca == 'miss')]
    epo_loc_undet_overall = epochs[((epochs.metadata.det == 'missF1') | (epochs.metadata.det == 'missF2')) &
                                   (epochs.metadata.loca == 'hit')]
    epo_noloc_undet_overall = epochs[((epochs.metadata.det == 'missF1') | (epochs.metadata.det == 'missF2')) &
                                     (epochs.metadata.loca == 'miss')]

    conditions = [epo_loc_anat, epo_noloc_anat, epo_loc_undet_anat, epo_noloc_undet_anat,
                  epo_loc_cross, epo_noloc_cross, epo_loc_undet_cross, epo_noloc_undet_cross,
                  epo_loc_overall, epo_noloc_overall, epo_loc_undet_overall, epo_noloc_undet_overall]
    ...
    av_epochs = [epochs_cond.average() for epochs_cond in conditions]
    ...
    subj_matrix = [epochs_cond.get_data() for epochs_cond in av_epochs]
    for i, m in enumerate(subj_matrix):
        matrices_for_matrix[i].append(m)

del epo_loc_anat, epo_noloc_anat, epo_loc_undet_anat, epo_noloc_undet_anat, epo_loc_cross, epo_noloc_cross, \
    epo_loc_undet_cross, epo_noloc_undet_cross, epo_loc_overall, epo_noloc_overall, epo_loc_undet_overall, \
    epo_noloc_undet_overall, av_epochs, conditions, cond, average, subj_matrix, i, m, subj

# For each condition get Grand Average
GrAvs = [mne.grand_average(evoked) for evoked in evokeds_for_GrAv]

# For each condition obtain data as a 3D matrix (from list of 2D matrices)
Evokeds = [np.array(i) for i in matrices_for_matrix]
